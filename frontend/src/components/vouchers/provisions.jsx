              ...
              <div className="border border-slate-200 rounded overflow-hidden">
                <table className="w-full text-xs text-left bg-white border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    ...
                    <td className="p-1 border-r">
                      <input 
                        type="number" step="any" value={it.credit || ""} 
                        onChange={e => { 
                          updateLine(it.id, "credit", e.target.value); 
                          if (parseFloat(e.target.value) > 0) updateLine(it.id, "debit", 0); 
                        }} 
                        className="w-full p-1.5 border border-transparent hover:border-slate-200 rounded font-mono text-right outline-none focus:border-indigo-500 bg-transparent text-orange-600 font-bold" 
                      />
                    </td>
                    <td className="p-1">
                      <input 
                        type="number" step="any" value={it.debit || ""} 
                        onChange={e => { 
                          updateLine(it.id, "debit", e.target.value); 
                          if (parseFloat(e.target.value) > 0) updateLine(it.id, "credit", 0); 
                        }} 
                        className="w-full p-1.5 border border-transparent hover:border-slate-200 rounded font-mono text-right outline-none focus:border-indigo-500 bg-transparent text-emerald-600 font-bold" 
                      />
                    </td>
