...
import { Plus, Trash, Printer, Edit2, X, FileText } from "lucide-react";
...
export default function ContraEntry({ database, onSaveVoucher, onDeleteVoucher }) {
  const mode = "CONTRA";
  ...
  const handlePrintVoucher = (v) => {
    ...
    const htmlContent = `
      <html>
        <head>
          <title>Contra Voucher - ${vNo}</title>
          <style>
        ...
        <body>
          ...
          <div class="print-wrapper">
            <div class="header-title">CONTRA VOUCHER</div>
  ...
  const handleSave = () => {
    ...
    const bankAccObj = database.accounts.find(a => a.name === bankAccount) || { code: "1001008", name: bankAccount };
    ...
    validLines.forEach(line => {
      doubleEntryLines.push({
        ...
        narration: narration || `Contra entry to ${line.accountName}`,
        debit: line.debit,
        credit: 0
      });
    });
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-[#F8FAFC]" id="contra-entry-viewport">
      <div className="p-6 bg-white border-b border-[#E2E8F0] flex items-center justify-between gap-4">
        <div>
          <h3 className="text-xs font-bold uppercase tracking-wider text-[#1E293B]">Contra Entry Register</h3>
          <p className="text-[10px] text-[#64748B] mt-1">Manage Cash-to-Bank and Bank-to-Cash liquid transfers.</p>
        </div>
        <button onClick={openNewVoucher} className="bg-[#2563EB] hover:bg-[#1D4ED8] text-white px-4 py-2 rounded text-xs font-semibold flex items-center gap-1.5 cursor-pointer">
          <Plus className="w-4 h-4" /> Add Contra Voucher
        </button>
      </div>
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-[#E2E8F0] text-[#64748B] font-bold uppercase tracking-wider">
                ...
                <th className="p-3">Counter Account</th>
              ...
              {vouchersList.map((v, index) => {
                ...
                return (
                  <tr key={v.id} onClick={() => loadVoucher(v)} className={`cursor-pointer ${index % 2 === 1 ? "bg-slate-50" : "bg-white"} hover:bg-blue-50`}>
                    ...
                    <td className="p-3 font-semibold text-slate-800">{firstLine.accountName || "Contra Entry"}</td>
      ...
      <div className={isModalOpen ? "fixed inset-0 z-50 bg-[#0F172A]/60 overflow-y-auto p-4 flex items-center justify-center" : "hidden"}>
        <div className="w-full max-w-3xl bg-white border border-[#E2E8F0] rounded-lg shadow-xl flex flex-col">
          <div className="px-6 py-4 bg-slate-50 border-b border-slate-200 flex justify-between items-center rounded-t-lg">
            <div>
              <span className="text-[10px] uppercase font-bold text-slate-400">Transfers</span>
              <h2 className="text-sm font-bold text-slate-800 uppercase tracking-wider">Contra Entry Voucher</h2>
            </div>
          ...
          <div className="p-6 flex flex-col gap-4 overflow-y-auto max-h-[80vh]">
            ...
            <div className="grid grid-cols-12 gap-x-4 gap-y-3 text-xs font-semibold text-[#1E293B] items-end">
              ...
              <div className="col-span-4 flex flex-col"><label className="text-[10px] uppercase font-bold text-[#64748B] mb-1">Cheque No</label><input type="text" value={chequeNo} onChange={(e) => setChequeNo(e.target.value)} placeholder="953919" className="p-2 border border-[#E2E8F0] rounded font-mono outline-none" /></div>
              <div className="col-span-4 flex flex-col"><label className="text-[10px] uppercase font-bold text-[#64748B] mb-1">Date</label><input type="date" value={chequeDate} onChange={(e) => setChequeDate(e.target.value)} className="p-2 border border-[#E2E8F0] rounded font-mono outline-none" /></div>
              <div className="col-span-4"></div>
            ...
            <div className="grid grid-cols-2 gap-4 text-xs mt-2">
              <div className="flex flex-col"><label className="text-[10px] uppercase font-bold text-[#64748B] mb-1">Cheque Name</label><input type="text" value={chequeName} onChange={(e) => setChequeName(e.target.value)} className="p-2 border border-[#E2E8F0] rounded font-mono outline-none" /></div>
            </div>
            ...
            <div className="flex items-center gap-4 mt-2">
              <label className="flex items-center gap-1.5 text-xs font-bold text-slate-700 cursor-pointer"><input type="checkbox" checked={acPay} onChange={(e) => setAcPay(e.target.checked)} className="rounded" /> A/C Pay</label>
              <button className="bg-slate-800 text-white px-3 py-1 rounded text-xs font-bold">Cheque</button>
            </div>
