...
export default function DebitNote({ database, onSaveVoucher, onDeleteVoucher, onPrint }) {
  ...
  useEffect(() => {
    const list = (database.vouchers || []).filter(v => v.type === "DN");
  ...
  const handleSave = () => {
    ...
    const payload = {
      ...
      billDate,
      type: "DN",
      narration: validLines[0]?.narration || "Debit Note Entry",
      items: validLines,
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-[#F8FAFC]">
      ...
      <div className="p-6 bg-white border-b border-[#E2E8F0] flex items-center justify-between gap-4">
        <div>
          <h3 className="text-xs font-bold uppercase tracking-wider text-[#1E293B]">Debit Note Register</h3>
          <p className="text-[10px] text-[#64748B] mt-1">Manage purchase returns, rate differences, and outstanding recoveries.</p>
        </div>
        <button onClick={openNewVoucher} className="bg-rose-600 hover:bg-rose-700 text-white px-4 py-2 rounded text-xs font-semibold flex items-center gap-1.5 shadow-sm">
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-[#E2E8F0]">
              {vouchersList.map((v, idx) => (
                <tr key={v.id} onClick={() => loadVoucher(v)} className={`cursor-pointer hover:bg-rose-50 ${idx % 2 === 1 ? 'bg-slate-50' : 'bg-white'}`}>
                  <td className="p-3 font-mono font-bold text-rose-600">{v.voucherNo}</td>
                  ...
                  <td className="p-3">
                    <div className="flex items-center justify-center gap-2">
                      <button onClick={(e) => { e.stopPropagation(); loadVoucher(v); }} className="text-slate-400 hover:text-rose-600"><Edit2 className="w-3.5 h-3.5" /></button>
              ...
              {vouchersList.length === 0 && (
                <tr><td colSpan={5} className="p-10 text-center text-slate-400 italic">No Debit Notes found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="fixed inset-0 z-50 bg-[#0F172A]/60 flex items-center justify-center p-4">
          <div className="w-full max-w-4xl bg-white border border-[#E2E8F0] rounded-lg shadow-xl flex flex-col max-h-[90vh]">
            ...
            <div className="px-6 py-4 bg-slate-50 border-b border-slate-200 flex justify-between items-center rounded-t-lg">
              <h2 className="text-sm font-bold text-slate-800 uppercase tracking-wider">{voucherId ? "Edit Debit Note" : "New Debit Note"}</h2>
            ...
            <div className="p-6 overflow-y-auto flex flex-col gap-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs font-semibold text-[#1E293B]">
                <div className="flex flex-col">
                  ...
                  <input type="text" value={voucherNo} onChange={e => setVoucherNo(e.target.value)} className="p-2 border border-slate-200 bg-slate-50 rounded font-mono outline-none focus:border-rose-500" />
                </div>
                <div className="flex flex-col">
                  <label className="text-[10px] uppercase font-bold text-[#64748B] mb-1">Date</label>
                  <input type="date" value={voucherDate} onChange={e => { setVoucherDate(e.target.value); setBillDate(e.target.value); }} className="p-2 border border-slate-200 rounded font-mono outline-none focus:border-rose-500" />
                </div>
                <div className="flex flex-col">
                  <label className="text-[10px] uppercase font-bold text-[#64748B] mb-1">Bill No</label>
                  <input type="text" value={billNo} onChange={e => setBillNo(e.target.value)} className="p-2 border border-slate-200 rounded outline-none focus:border-rose-500" />
                </div>
                <div className="flex flex-col">
                  <label className="text-[10px] uppercase font-bold text-[#64748B] mb-1">Date</label>
                  <input type="date" value={billDate} onChange={e => setBillDate(e.target.value)} className="p-2 border border-slate-200 font-mono rounded outline-none focus:border-rose-500" />
                </div>
              ...
              <div className="border border-slate-200 rounded overflow-hidden">
                    ...
                    {items.map((it, idx) => (
                      <tr key={it.id} className="hover:bg-slate-50">
                        <td className="p-1 border-r">
                          <select value={it.accountCode} onChange={e => updateLine(it.id, "accountCode", e.target.value)} className="w-full p-1.5 border border-transparent hover:border-slate-200 rounded outline-none focus:border-rose-500 bg-transparent font-semibold">
                            <option value="">-- Select Ledger --</option>
                            {database.accounts.map(a => <option key={a.code} value={a.code}>{a.name}</option>)}
                          </select>
                        </td>
                        <td className="p-1 border-r"><input type="text" value={it.narration} onChange={e => updateLine(it.id, "narration", e.target.value)} className="w-full p-1.5 border border-transparent hover:border-slate-200 rounded outline-none focus:border-rose-500 bg-transparent" placeholder="Particulars..." /></td>
                        <td className="p-1 border-r"><input type="number" step="any" value={it.credit || ""} onChange={e => { updateLine(it.id, "credit", e.target.value); if(parseFloat(e.target.value) > 0) updateLine(it.id, "debit", 0); }} className="w-full p-1.5 border border-transparent hover:border-slate-200 rounded font-mono text-right outline-none focus:border-rose-500 bg-transparent text-orange-600 font-bold" /></td>
                ...
                <div className="bg-slate-50 p-1.5 border-t border-slate-200 flex items-center justify-between">
                  <button onClick={addLine} className="text-[11px] text-rose-600 font-bold hover:underline ml-2">+ Add Row</button>
                </div>
              </div>
            ...
            <div className="px-6 py-4 border-t border-slate-200 bg-slate-50 flex justify-between items-center rounded-b-lg">
              ...
              <div className="flex gap-2">
                <button onClick={handleSave} className="bg-rose-600 hover:bg-rose-700 text-white px-8 py-2 rounded text-xs font-bold shadow-sm transition-colors">Save</button>
